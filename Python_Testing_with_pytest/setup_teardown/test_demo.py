...
# （类中）方法和函数级别（Method and function level setup/teardown）：作用于单个测试用例，若用例没有执行（如被skip了）或失败了，则不会执行teardown
...
# pytest.fixture()装饰函数，结合yield实现初始化和teardown举个例子(pytest）文档中的：
...
# # 1、setup_method(self, method)在test_method.py中构建了3个测试用例，每个用例在执行前后都会执行setup_method/teardown_method连接smtp和断开smtp。
# import smtplib
