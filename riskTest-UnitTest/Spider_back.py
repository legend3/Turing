...
from Normal import NormalThread
                    ...
                    if content:
                        ...
                        for j in range(content_length):
                            listBody += "{\"user\": \"晓红\", \"phone\": \"%d\", \"email\": \"%d@163.com\", \"card\":\"6227002470170278192\", \"pid\": \"320481199004101010\"}," % (
                                15365012832 + i + j, 15365012832 + i + j)
                        # end for
                    ...
                    if customerDetail:
                        ...
                        if content:
                            ...
                            detailBody += "{\"user\": \"晓红\", \"phone\": \"%d\", \"email\": \"%d@163.com\", \"card\":\"6227002470170278192\", \"pid\": \"320481199004101010\"}" % (
                                15365013832 + i + k, 15365011832 + i + k)
                            detailBody += "}"
...
#             elif self.threadName == "dayAmountOverNormal":
#                 # 日访问次数偏移用户基线
#                 apiEventList = self.runDayAmountOverNormal(self.tm, "192.168.2.10", "gotbiq")
#             elif self.threadName == "dayAmountLargeOverNormal":
#                 # 日访问次数大幅偏移用户基线
#                 apiEventList = self.runDayAmountLargeOverNormal(self.tm, "192.168.3.22", "ukeqqx")
#             elif self.threadName == "dayLabelCountOverNormal":
#                 # 日敏感标签访问量偏移用户基线
#                 apiEventList = self.runDayLabelCountOverNormal(self.tm, "192.168.4.34", "hwvuat")
#             elif self.threadName == "dayLabelCountLargeOverNormal":
#                 # 日敏感标签访问量大幅偏移用户基线
#                 apiEventList = self.runDayLabelCountLargeOverNormal(self.tm, "192.168.5.50", "ewlwrt")
#             elif self.threadName == "ipPropsOverNormal":
#                 # 使用IP属性偏移用户基线
#                 apiEventList = self.runIpPropsOverNormal(self.tm, "220.184.86.203", "xxjxzo")
#             elif self.threadName == "ipPropsLargeOverNormal":
#                 # 使用IP属性大幅偏移用户基线
#                 apiEventList = self.runIpPropsOverNormal(self.tm, "220.184.86.203", "uibixg")
#                 apiEventList.extend(self.runIpPropsOverNormal(self.tm, "211.134.100.100", "uibixg"))
#                 apiEventList.extend(self.runIpPropsOverNormal(self.tm, "56.23.52.41", "uibixg"))
#             elif self.threadName == "ipCountOverNormal":
#                 # 使用IP数量偏移用户基线
#                 apiEventList = self.runNormal(self.tm, "192.168.6.101", "qlgwas")
#                 apiEventList.extend(self.runNormal(self.tm, "192.168.6.200", "qlgwas"))
...
#             elif self.threadName == "circlrAmount":
#                 # 是否周期性访问异常
#                 apiEventList = self.runCirclrAmount(self.tm, "192.168.4.34", "gbrpfa")
#             elif self.threadName == "labelCountPreOverNormal":
#                 # 单次请求数据量偏离基线
#                 apiEventList = self.runLabelCountPreOverNormal(self.tm, "192.168.7.120", "thwyrq")
#             elif self.threadName == "amountFreOverNormal":
#                 # 请求频次偏移基线
#                 apiEventList = self.runAmountFreOverNormal(self.tm, "192.168.8.141", "unqzdk")
#             elif self.threadName == "apiRouteOverNormal":
#                 # 接口请求路径偏离基线
#                 apiEventList = self.runApiRouteOverNormal(self.tm, "192.168.4.34", "zmefsx")
#             elif self.threadName == "cookieUseIps":
#                 # 同一个Cookie在不同的IP上使用
#                 apiEventList = self.runCookieUseIps(self.tm, "myquzl", ["192.168.0.100", "192.168.1.101", "192.168.4.34"])
#             elif self.threadName == "ipRangeExcept":
#                 # IP地址段异常，出现以前没出现过的IP段
#                 apiEventList = self.runIpRangeExcept(self.tm, "cqtuco", "10.10.0.159")
#             elif self.threadName == "exceptTime":
#                 # 异常时间段
#                 apiEventList = self.runExceptTime(self.tm, "gwfakc", "10.97.13.93")
#             elif self.threadName == "oneByOne":
...
def main(hosts, time_str):
    try:
        ...
        threadNameList = ["ipRangeExcept",
                         "dayAmountOverNormal", "dayAmountLargeOverNormal", "dayLabelCountOverNormal",
                          "dayLabelCountLargeOverNormal", "ipPropsOverNormal", "ipPropsLargeOverNormal",
                          "ipCountOverNormal", "circlrAmount", "ipOverUserCount", "labelCountPreOverNormal",
                          "amountFreOverNormal", "apiRouteOverNormal", "exceptTime"]
...
if __name__ == '__main__':
    ...
    if len(sys.argv) != 2:
        print "argv error, RiskTest.py ip:port"
        sys.exit(1)
    # end if

    hosts = sys.argv[1]

    # nortmal_thread = NormalThread()

    #1、跑配置
    # start_mongo_config(hosts)
    # start_date="2018-08-01"
    # test_pan=29
    # pre_data_main(start_date,test_pan,hosts)

    #3、跑异常，之前先跑Normal
    main(hosts, "2018-09-03 00:00:00")
