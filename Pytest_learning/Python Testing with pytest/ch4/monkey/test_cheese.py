...
'''
monkeypath主要有以下几个用处：(个人理解,monkeypatch像一个全局的管理者,可随时随地对任何环境进行修改 配置;只是修改了执行时的代码,不破坏被调用的原代码)
    在运行时替换方法、属性等
...
• syspath_prepend(path): Prepend path to sys.path, which is Python’s list of import locations.(设置添加系统路径)
