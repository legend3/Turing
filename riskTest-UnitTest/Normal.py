...
class NormalThread(threading.Thread):
    def __init__(self, topic, users_set, dateList,apiList,hourList,apiConfig,apiData):
        try:
                        ...
                        for u in self.users:
                            ...
                            for t_e in self.dateList:
                                ...
                                if xinqi == 6 or xinqi == 0:
                                    ...
                                    api_num = sam_simple_po(1, -1, 1, 2, 1)[0]
                                    # user_list_now = RandomSampling(user_list_ana, user_n_weekend)
                                    # print len(user_list_now)


                                else:
                                    api_num = sam_simple_po(1, 2.3, 0.3, api_top_num-1, 1)[0]
                                    user_flag = True
                                    # print len(api_num)
                                ...
                                if user_flag:
                                        ...
                                        hour_now_list = self.choice_time_during(self.hourList)

                                        ip_now = [ip]

                                        # 根据用户访问强度和日期访问强度，生成综合访问强度power
                                        power = user_power - date_power
                                        api_list_now = RandomSampling(self.apiList[1:], api_num)
                                        api_list_now.append(1)
                                        api_list_now.sort()
                                        # print api_list_now
                                        api_power = 0
                                        for api in api_list_now:
                                            ...
                                            batch_num = sam_simple_po(1, 1.7, 0.3, 3, 1)[0]
                                            # print batch_num
                                            ...
                                            if batch_time:
                                                ...
                                                for j in xrange(len(batch_time)):
                                                    ...
                                                    power_b = (power -  1.2*j) * 0.45 + api_power

                                                    if api_seq == 3:
                                                        power_b=2
                                                    batch_clock = batch_time[j]

                                                    batch_long = sam_simple_po(1, 11, 5, 22, 2)[0]
                                                    # print batch_long

                                                    if flag:
                                                        batch_long = 1
                                                        power_b = -200
                                                    rr = self.time_s(RandomSampling, batch_clock, batch_long, power_b, riqi)
                                                    ...
                                                    for each in rr:
                                                        ii += 1
                                                        oi = ii % 8900 + 1000
                                                        info=self.apiData(api_seq,oi)
                                                        url=info["url"]
                                                        body=info["body"]
                                                        # print user
                                                        a =self.apiConfig(url=url, tm=each, ip=ip_now_e, user=user,body=body)
                                                        output_infos.append(a)

                                                    self.send(output_infos)
...
def initIpList():
    try:
        ...
        data_add = ['84.235.122.205', '202.100.073.255', '202.101.104.031', '202.101.096.127',
...
def main_norml(hosts,threadNum,start_day,len_pan):
    try:
        ...
        for i in range(threadNum):
            # broker,
            thread_list.append(NormalThread(topic,users_set[i], dateList, apiList,hourList,apiconfig,apiData))
        #end for
