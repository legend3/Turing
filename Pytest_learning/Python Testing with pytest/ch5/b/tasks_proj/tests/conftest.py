...
'''
1.
插件的作用：第三方插件包含相当多的代码。这就是我们使用它们来节省我们自己开发所有代码的时间的原因之一。然而，对于您特定的编码域，您无疑会提出特殊的fixture和修改来帮助您进行测试。通过创建插件，您可以轻松地在几个项目之间共享少量的fixture
插件定义描述：插件可以包含改变pytest行为的hook函数。因为pytest的目的是允许插件改变pytest的行为方式，所以有很多hook函数可用。pytest的hook函数被指定在pytest文档站点上
插件的创建布局模式(不一定，但很调理)：为了将行为变化与插件机制的讨论分开，在将其转换为可分发的插件之前我们将我们的修改放在conftest.py中；那样修改(插件)不仅仅在一个项目中被使用而且将变得可以共享、扩展

2.
pytest.config global
    Removed in version 5.0.
pytest.config global对象已被废弃：
a.如果是非插件(conftest.py中hook)则可以用request.config(通过fixture里的request)
b.或者如果你是插件作者，使用pytest_configure(config)hook。
*c.注意！许多hook也可以间接使用config对象通过session.config或者item.config为测试用例。
'''


def pytest_report_header():  # hook函数
...
nodeid,规范化收集节点id
location,一个(filesystempath, lineno, domaininfo)元组表示一个测试项的实际位置——它可能与收集到的不同，例如，如果一个方法是从另一个模块继承来的。
...
def pytest_report_teststatus(report):  # hook函数
